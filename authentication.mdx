...
Modem uses API keys to authenticate requests, ensuring that only authorized applications can access payment functions. API keys are generated in the Modem dashboard and can be used in either live or test modes.

1. **API Keys**  
   Modem provides two types of API keys:
...
3. **Test and Live Modes**
   ...
   Modem separates test and live keys, enabling you to test transactions without impacting real accounts. Test keys operate in a sandbox environment, allowing you to simulate payment flows before going live.
