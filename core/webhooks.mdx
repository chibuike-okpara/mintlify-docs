...
*Note: You can generate your unique Modem webhook secret in the Modem dashboard under **Developers > Webhooks**.*


### Setting Up Webhooks

1. **Define Your Webhook Endpoint**:  
   Create a dedicated `POST` endpoint on your server to receive webhook events from Modem. Ensure this endpoint is accessible over HTTPS to maintain security.
...
To confirm the integrity of incoming events, Modem includes a unique `x-modem-signature` header in each webhook request. This header allows you to verify that the event originated from Modem.
...
2. **Use `modem.webhooks.composeEventDetails` for Validation**:  
   Pass the payload, signature, and your unique Modem secret to the `modem.webhooks.composeEventDetails` function. This function performs the necessary checks, verifying the signature and parsing the payload.
...
For each webhook event, respond to Modem with an HTTP `200 OK` status as confirmation. This ensures Modem won’t re-send the event unless there's an error response.

### Summary

By following these steps, you can securely receive and process webhook events from Modem, automating actions based on customer interactions and payment flows. Make sure to store and securely handle the event data, as each event may carry critical transaction or customer information relevant to your application.
