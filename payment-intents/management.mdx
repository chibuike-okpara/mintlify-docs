...
The Modem SDK provides straightforward methods to manage Payment Intents beyond creating them, allowing you to cancel, retrieve, and list Payment Intents easily. 
...
<CodeGroup>
```javascript nodejs
const paymentIntent = await modem.paymentIntents.cancel(
...
<CodeGroup>
```javascript nodejs
const paymentIntent = await modem.paymentIntents.retrieve(
...
<CodeGroup>
```javascript nodejs
const paymentIntents = await modem.paymentIntents.list({ limit: 10 });
```
</CodeGroup>
